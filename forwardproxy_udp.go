...
func newUDPProxyServer(uri string) (*udpProxyServer, error) {
	srv := &udpProxyServer{}
	if uri == "" {
	...
	if err != nil {
		return nil, fmt.Errorf("parse uri template error: %w", err)
	}
	return srv, err
}
...
func (srv *udpProxyServer) HandleStream(c io.ReadWriter, req Request, rc *net.UDPConn) error {
	...
	for {
		...
		// data.Length = pl.Len(0) + uint64(nr)
...
func (srv *udpProxyServer) HandlePacket(str http3.Stream, req Request, c *net.UDPConn) error {
	// https://github.com/quic-go/masque-go/issues/64
	if req == "*" {
		return srv.HandlePacketBind(str, req, c)
	}
	...
	go func() {
		...
		for {
			...
			nr := 8 - quicvarint.Len(0)
			quicvarint.Append(b[:nr], 0)

			if err := str.SendDatagram(b[nr : 8+n]); err != nil {
				break
			}
		}
...
func (srv *udpProxyServer) ParseRequest(r *http.Request) (Request, error) {
	...
	match, err := srv.Matcher.Extract(r.URL.String())
	if err != nil {
		return "", fmt.Errorf("extract uri error: %w", err)
	}
