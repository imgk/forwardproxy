...
func TestHandleStream(t *testing.T) {
	...
	t.Run("test handle stream bind", func(t *testing.T) {
		...
		t.Run("test uncompression payload", func(t *testing.T) {
			...
			dg.Length = 1 + 1 + 4 + 2 + uint64(len(hello))
			dg.Payload = &pl
			...
			func() {
				...
				if plr.ContextID != 1 {
					t.Errorf("parse compression assign context id error: want: 1, get: %v", plr.ContextID)
				}
				if plr.IPVersion != pl.IPVersion {
					// t.Errorf("parse compression assign IP version error: want: %v, get: %v", pl.IPVersion, plr.IPVersion)
				}
				if plr.Addr != raddr.Addr() {
					// t.Errorf("parse compression assign addr error: want: %v, get: %v", pl.Addr, raddr.Addr())
				}
			...
			if dg.Length != dgr.Length {
				// t.Errorf("datagram length error: want: %v, get: %v", dg.Length, dgr.Length)
			}
			if plr.IPVersion != pl.IPVersion {
				// t.Errorf("parse uncompressed error: want: %v, get: %v", pl.IPVersion, plr.IPVersion)
			}
			if plr.Addr != raddr.Addr() {
				// t.Errorf("parse uncompressed error: want: %v, get: %v", pl.Addr, raddr.Addr())
			}
			if plr.Port != raddr.Port() {
				t.Errorf("parse uncompressed error: want: %v, get: %v", pl.Port, raddr.Port())
			}
...
func TestParseRequst(t *testing.T) {
	t.Run("test parse request", func(t *testing.T) {
		...
		for _, v := range tests {
			srv, err := newUDPProxyServer(v.Template)
			if err != nil {
